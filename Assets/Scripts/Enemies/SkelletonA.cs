...
public class SkelletonA : Enemy
{
    ...
    void Start()
    {
        ...
        attack = GetComponentInChildren<ArrowAttack>();
    }
    ...
    void FixedUpdate()
    {
        if (!isDead)
        {
            ...
            if (attackAllowed && Mathf.Abs(playerDistance.x) < 10 && Mathf.Abs(playerDistance.y) < 3)
            {
                ...
                ArrowAttack newAttack = Instantiate(attack, attack.transform.position, Quaternion.identity);
